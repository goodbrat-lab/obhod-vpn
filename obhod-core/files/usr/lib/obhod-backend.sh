...
check_required_file "$OBHOD_LIB/constants.sh"
check_required_file "$OBHOD_LIB/nft.sh"
check_required_file "$OBHOD_LIB/helpers.sh"
check_required_file "$OBHOD_LIB/sing_box_config_manager.sh"
...
. "$OBHOD_LIB/sing_box_config_facade.sh"
...
check_requirements() {
    ...
    local sing_box_version jq_version coreutils_base64_version nft_version dnsmasq_version
    
    # Get versions with error handling
    sing_box_version="$(sing-box version 2>/dev/null | head -n1 | awk '{print $3}')"
    jq_version="$(jq --version 2>/dev/null | awk -F- '{print $2}')"
    coreutils_base64_version="$(base64 --version 2>/dev/null | head -n1 | awk '{print $4}')"
    nft_version="$(nft -v 2>/dev/null | awk '{print $2}')"
...
_append_proxy_link() {
    local link="$1"
    if [ -z "$_proxy_links" ]; then
        _proxy_links="$link"
    else
        _proxy_links="$_proxy_links $link"
    fi
}

_check_outbound_section() {
    local section="$1"
    local proxy_string interface outbound_json urltest_proxy_links subscription_url selector_proxy_links

    config_get proxy_string "$section" "proxy_string"
    
    _proxy_links=""
    config_list_foreach "$section" "selector_proxy_links" _append_proxy_link
    selector_proxy_links="$_proxy_links"
    
    _proxy_links=""
    config_list_foreach "$section" "urltest_proxy_links" _append_proxy_link
    urltest_proxy_links="$_proxy_links"
    ...
    if [ -n "$proxy_string" ] || [ -n "$selector_proxy_links" ] || [ -n "$urltest_proxy_links" ] ||
        [ -n "$outbound_json" ] || [ -n "$interface" ] || [ -n "$subscription_url" ]; then
        section_exists=0
    fi
}
...
fetch_subscription() {
    ...
    if echo "$data" | grep -qiE '^[[:space:]]*(vless|vmess|trojan|ss|hy2|hysteria2|socks[45])://'; then
        ...
        # Manually add padding if missing (some base64 decoders are picky)
        mod=$((${#cleaned_data} % 4))
        if [ "$mod" -eq 2 ]; then
    ...
    # Convert CRLF to LF and remove trailing spaces from each line
    if [ -f "$output_file" ]; then
        tr -d '\r' < "$output_file" | sed 's/[[:space:]]*$//' > "${output_file}.tmp" && mv "${output_file}.tmp" "$output_file"
    fi
...
stop_main() {
    ...
    # Note: watchdog is managed by procd and killed automatically.
    # We clean up any stale PID file from old manual launches just in case.
    rm -f /var/run/obhod_watchdog.pid

    remove_cron_job

    rm -f "$TMP_RULESET_FOLDER"/*

    # Restore dnsmasq before stopping sing-box
    local dont_touch_dhcp
    config_get_bool dont_touch_dhcp "settings" "dont_touch_dhcp" 0
    if [ "$dont_touch_dhcp" -eq 0 ]; then
...
add_cron_job() {
    ...
    "1h")
        cron_job="13 * * * * /usr/bin/obhod list_update"
        ;;
    "3h")
        cron_job="13 */3 * * * /usr/bin/obhod list_update"
        ;;
    "12h")
        cron_job="13 */12 * * * /usr/bin/obhod list_update"
        ;;
    "1d")
        cron_job="13 9 * * * /usr/bin/obhod list_update"
        ;;
    "3d")
...
configure_outbound_handler() {
    ...
    proxy)
        ...
        subscription)
            ...
            local tmp_sub="/tmp/obhod_sub_$section.txt"
            if fetch_subscription "$sub_url" "$tmp_sub"; then
                # Filter only lines that look like proxy links
                ...
                local urltest_tag selector_tag outbound_tags urltest_outbounds selector_outbounds
                local urltest_check_interval urltest_tolerance urltest_testing_url
                
                config_get urltest_check_interval "$section" "subscription_update_interval" "3m"
                ...
                for link in $sub_links; do
                    ...
                    local current_tag="$section-sub-$i"
                    local new_config
                    new_config=$(sing_box_cf_add_proxy_outbound "$config" "$section" "$link" "0" "$current_tag" 2>&1)
                    if [ -n "$new_config" ] && ! printf '%s' "$new_config" | grep -q '^parse error:'; then
                        config="$new_config"
                        if [ -z "$outbound_tags" ]; then
                            outbound_tags="$current_tag"
                        else
                            outbound_tags="$outbound_tags,$current_tag"
                        fi
                    else
...
sing_box_config_check() {
    ...
    if ! sing-box -c "$config_path" check > "$check_log" 2>&1; then
        local error_msg
        error_msg=$(cat "$check_log")
        
        # Clean up the error message from sing-box FATAL prefix and paths
        local clean_error
...
get_download_detour_tag() {
    ...
    if [ "$download_lists_via_proxy" -eq 1 ]; then
        ...
        if [ -z "$download_lists_via_proxy_section" ]; then
            download_lists_via_proxy_section="$(get_first_outbound_section)"
        fi
        
        if [ -n "$download_lists_via_proxy_section" ]; then
            section_outbound_tag="$(get_outbound_tag_by_section "$download_lists_via_proxy_section")"
...
_determine_first_outbound_section() {
    ...
    if [ "$connection_type" = "proxy" ] || [ "$connection_type" = "vpn" ] || [ "$connection_type" = "urltest" ] || [ "$connection_type" = "selector" ]; then
        [ -z "$first_outbound_section" ] && first_outbound_section="$section"
    fi
}

get_first_outbound_section() {
    export first_outbound_section=""
    config_foreach _determine_first_outbound_section "section"
    echo "$first_outbound_section"
}
get_sections_by_connection_type() {
    local connection_type="$1"

    # Validate connection_type to prevent injection
    case "$connection_type" in
        proxy|vpn|block|exclusion|urltest|selector)
...
section_by_connection_type_exists() {
    ...
    if uci show obhod | grep -q "\.connection_type='$connection_type'"; then
        return 0
    else
        return 1
    fi
...
check_nft() {
    ...
    if [ "$found_hetzner" -eq 1 ] || [ "$found_ovh" -eq 1 ]; then
        ...
        local sets="obhod_subnets obhod_domains interfaces obhod_discord_subnets localv4"
        ...
        for set_name in $sets; do
            if nft list set inet "$NFT_TABLE_NAME" "$set_name" > /dev/null 2>&1; then
                ...
                count=$(nft list set inet "$NFT_TABLE_NAME" "$set_name" 2> /dev/null | grep -o ',\|{' | wc -l)
...
check_logs() {
    ...
    local logs
    logs=$(logread | grep -E "obhod|sing-box")
    ...
    # Find the last occurrence of "Starting obhod"
    local start_line
    start_line=$(echo "$logs" | grep -n "obhod.*Starting obhod" | tail -n 1 | cut -d: -f1)

    if [ -n "$start_line" ]; then
        echo "$logs" | tail -n +"$start_line"
    else
        nolog "No 'Starting obhod' message found, showing last 100 lines"
...
get_system_info() {
    local obhod_version obhod_latest_version luci_app_version sing_box_version openwrt_version device_model

    obhod_version="$OBHOD_VERSION"

    obhod_latest_version=$(curl -m 3 -s https://api.github.com/repos/itdoginfo/obhod/releases/latest | grep '"tag_name":' | cut -d'"' -f4)
    [ -z "$obhod_latest_version" ] && obhod_latest_version="unknown"

    if [ -f /www/luci-static/resources/view/obhod/main.js ]; then
        luci_app_version=$(grep 'var OBHOD_LUCI_APP_VERSION' /www/luci-static/resources/view/obhod/main.js | cut -d'"' -f2)
    else
    ...
    echo "{\"obhod_version\": \"$obhod_version\", \"obhod_latest_version\": \"$obhod_latest_version\", \"luci_app_version\": \"$luci_app_version\", \"sing_box_version\": \"$sing_box_version\", \"openwrt_version\": \"$openwrt_version\", \"device_model\": \"$device_model\"}" | jq .
}
...
check_nft_rules() {
    ...
    # Generate traffic through ObhodTable
    ...
    # Check for other mark rules outside ObhodTable
    nft list tables 2> /dev/null | while read -r _ family table_name; do
        ...
        if nft list table "$family" "$table_name" 2> /dev/null | grep -q "meta mark set"; then
            touch /tmp/obhod_mark_check.$$
            break
        fi
    done

    if [ -f /tmp/obhod_mark_check.$$ ]; then
        rules_other_mark_exist=1
        rm -f /tmp/obhod_mark_check.$$
    fi
...
global_check() {
    local OBHOD_LUCI_VERSION="Unknown"
    [ -n "$1" ] && OBHOD_LUCI_VERSION="$1"
    ...
    if [ -n "$system_info_json" ]; then
        ...
        obhod_version=$(echo "$system_info_json" | jq -r '.obhod_version // "unknown"')
        obhod_latest_version=$(echo "$system_info_json" | jq -r '.obhod_latest_version // "unknown"')
        ...
        print_global "🕳️ Obhod:        $obhod_version (latest: $obhod_latest_version)"
...
Available commands:
    start                   Start obhod service
    stop                    Stop obhod service
    reload                  Reload obhod configuration
    restart                 Restart obhod service
    validate                Validate configuration
    main                    Run main obhod process
    ...
    check_logs              Show obhod logs from system journal
    ...
    show_config             Display current obhod configuration
    show_version            Show obhod version
    show_sing_box_config    Show sing-box configuration
    show_sing_box_version   Show sing-box version
    show_system_info        Show system information
    get_status              Get obhod service status
...
health)
    if [ -x "/usr/bin/obhod" ]; then
        /usr/bin/obhod health
    else
        echo '{"error":"obhod binary not found"}'
    fi
    ;;
auto_setup)
    if [ -x "/usr/bin/obhod" ]; then
        /usr/bin/obhod auto-setup
    else
        echo '{"error":"obhod binary not found"}'
    fi
    ;;
