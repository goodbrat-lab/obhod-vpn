...
function validateVlessUrl(url) {
  try {
    ...
    if (!host)
      return { valid: false, message: _("Invalid VLESS URL: missing hostname") };
    if (!port)
    ...
    if (params.flow === "xtls-rprx-vision-udp443") {
      return {
        valid: false,
        message: _("Invalid VLESS URL: flow xtls-rprx-vision-udp443 is not supported")
      };
    }
    return { valid: true, message: _("Valid") };
  } catch (_e) {
...
function validateTrojanUrl(url) {
  try {
    ...
    if (!userPart)
      return { valid: false, message: _("Invalid Trojan URL: missing password") };
    if (!hostPortPart)
      return {
        valid: false,
    ...
    const [host, port] = hostPortPart.split(":");
    if (!host)
      return { valid: false, message: _("Invalid Trojan URL: missing hostname") };
    if (!port)
...
// src/obhod/methods/custom/getConfigSections.ts
async function getConfigSections() {
  return uci.load("obhod").then(() => uci.sections("obhod"));
}

// src/obhod/methods/shell/callBaseMethod.ts
async function callBaseMethod(method, args = [], command = "/usr/bin/obhod") {
...
var ObhodShellMethods = {
  checkDNSAvailable: async () => callBaseMethod(
    Obhod.AvailableMethods.CHECK_DNS_AVAILABLE
  ),
  checkFakeIP: async () => callBaseMethod(
    Obhod.AvailableMethods.CHECK_FAKEIP
  ),
  checkNftRules: async () => callBaseMethod(
    Obhod.AvailableMethods.CHECK_NFT_RULES
  ),
  getStatus: async () => callBaseMethod(Obhod.AvailableMethods.GET_STATUS),
  checkSingBox: async () => callBaseMethod(
    Obhod.AvailableMethods.CHECK_SING_BOX
  ),
  ...
  getClashApiProxyLatency: async (tag) => callBaseMethod(
    Obhod.AvailableMethods.CLASH_API,
    [Obhod.AvailableClashAPIMethods.GET_PROXY_LATENCY, tag, "5000"]
  ),
  getClashApiGroupLatency: async (tag) => callBaseMethod(
    Obhod.AvailableMethods.CLASH_API,
    [Obhod.AvailableClashAPIMethods.GET_GROUP_LATENCY, tag, "10000"]
  ),
  setClashApiGroupProxy: async (group, proxy) => callBaseMethod(Obhod.AvailableMethods.CLASH_API, [
    Obhod.AvailableClashAPIMethods.SET_GROUP_PROXY,
    group,
    proxy
  ]),
  restart: async () => callBaseMethod(
    Obhod.AvailableMethods.RESTART,
    [],
    "/etc/init.d/obhod"
  ),
  start: async () => callBaseMethod(
    Obhod.AvailableMethods.START,
    [],
    "/etc/init.d/obhod"
  ),
  stop: async () => callBaseMethod(
    Obhod.AvailableMethods.STOP,
    [],
    "/etc/init.d/obhod"
  ),
  enable: async () => callBaseMethod(
    Obhod.AvailableMethods.ENABLE,
    [],
    "/etc/init.d/obhod"
  ),
  disable: async () => callBaseMethod(
    Obhod.AvailableMethods.DISABLE,
    [],
    "/etc/init.d/obhod"
  ),
  globalCheck: async () => callBaseMethod(Obhod.AvailableMethods.GLOBAL_CHECK),
  showSingBoxConfig: async () => callBaseMethod(Obhod.AvailableMethods.SHOW_SING_BOX_CONFIG),
  checkLogs: async () => callBaseMethod(Obhod.AvailableMethods.CHECK_LOGS),
...
async function getDashboardSections() {
  ...
  const clashProxies = await ObhodShellMethods.getClashApiProxies();
...
// src/obhod/methods/custom/getClashApiSecret.ts
...
// src/obhod/methods/custom/index.ts
var CustomObhodMethods = {
  getConfigSections,
...
var OBHOD_LUCI_APP_VERSION = "1.1.20";
...
// src/obhod/methods/fakeip/getFakeIpCheck.ts
...
// src/obhod/methods/fakeip/getIpCheck.ts
...
// src/obhod/methods/fakeip/index.ts
var RemoteFakeIPMethods = {
  getFakeIpCheck,
  getIpCheck
};

// src/obhod/services/tab.service.ts
...
// src/obhod/tabs/diagnostic/helpers/getCheckTitle.ts
function getCheckTitle(name) {
  return `${name} ${_("checks")}`;
}

// src/obhod/tabs/diagnostic/checks/contstants.ts
...
// src/obhod/tabs/diagnostic/diagnostic.store.ts
var initialDiagnosticStore = {
  diagnosticsSystemInfo: {
    loading: true,
    obhod_version: "loading",
    obhod_latest_version: "loading",
...
// src/obhod/services/store.service.ts
...
// src/obhod/services/logger.service.ts
var Logger = class {
  constructor() {
    this.logs = [];
...
// src/obhod/services/obhodLogWatcher.service.ts
var ObhodLogWatcher = class _ObhodLogWatcher {
  constructor() {
    this.intervalMs = 5e3;
  ...
  static getInstance() {
    if (!_ObhodLogWatcher.instance) {
      _ObhodLogWatcher.instance = new _ObhodLogWatcher();
    }
    return _ObhodLogWatcher.instance;
  }
  ...
  async checkOnce() {
    ...
    if (this.paused) {
      logger.debug("[ObhodLogWatcher]", "skipped check \u2014 tab not visible");
      return;
    }
    try {
      ...
      logger.error("[ObhodLogWatcher]", "failed to read logs:", err);
    }
  }
  start() {
    if (this.running) return;
    if (!this.fetcher) {
      logger.warn("[ObhodLogWatcher]", "attempted to start without fetcher");
      return;
    }
    this.running = true;
  ...
  stop() {
    ...
    logger.info("[ObhodLogWatcher]", "stopped");
  }
  pause() {
    if (!this.running || this.paused) return;
    this.paused = true;
    logger.info("[ObhodLogWatcher]", "paused (tab not visible)");
  }
  resume() {
    if (!this.running || !this.paused) return;
    this.paused = false;
    logger.info("[ObhodLogWatcher]", "resumed (tab active)");
    this.checkOnce();
  }
  reset() {
    this.lastLines.clear();
    logger.info("[ObhodLogWatcher]", "log history reset");
  }
};

// src/obhod/services/core.service.ts
function coreService() {
  ...
  const watcher = ObhodLogWatcher.getInstance();
  watcher.init(
    async () => {
      const logs = await ObhodShellMethods.checkLogs();
      if (logs.success) {
        return logs.data;
      }
      return "";
...
// src/obhod/tabs/dashboard/partials/renderSections.ts
...
// src/obhod/tabs/dashboard/partials/renderWidget.ts
...
// src/obhod/tabs/dashboard/render.ts
function render() {
  return E(
    "div",
    {
  ...
  if (obhod.success && singbox.success) {
    store.set({
      servicesInfoWidget: {
        loading: false,
        failed: false,
        data: { singbox: singbox.data.running, obhod: obhod.data.enabled }
      }
    });
  }
}

// src/obhod/tabs/dashboard/initController.ts
async function fetchDashboardSections() {
  ...
  const { data, success } = await CustomObhodMethods.getDashboardSections();
  if (!success) {
...
async function handleTestGroupLatency(tag) {
  ...
  await ObhodShellMethods.getClashApiGroupLatency(tag);
  await fetchDashboardSections();
  store.set({
    sectionsWidget: {
      ...store.get().sectionsWidget,
...
async function handleTestProxyLatency(tag) {
  ...
  await ObhodShellMethods.getClashApiProxyLatency(tag);
  await fetchDashboardSections();
  store.set({
    sectionsWidget: {
      ...store.get().sectionsWidget,
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: _("Obhod"),
        value: servicesInfoWidget.data.obhod ? _("\u2714 Enabled") : _("\u2718 Disabled"),
        attributes: {
          class: servicesInfoWidget.data.obhod ? "pdk_dashboard-page__widgets-section__item__row--success" : "pdk_dashboard-page__widgets-section__item__row--error"
        }
      },
      {
        key: _("Sing-box"),
...
// src/obhod/tabs/dashboard/styles.ts
var styles = `
#cbi-obhod-dashboard-_mount_node > div {
    width: 100%;
}

#cbi-obhod-dashboard > h3 {
    display: none;
}
...
// src/obhod/tabs/dashboard/index.ts
var DashboardTab = {
  render,
  initController,
  styles
...
// src/obhod/tabs/diagnostic/renderDiagnostic.ts
function render2() {
...
// src/obhod/tabs/diagnostic/checks/updateCheckStore.ts
...
// src/obhod/tabs/diagnostic/helpers/getMeta.ts
...
// src/obhod/tabs/diagnostic/checks/runDnsCheck.ts
async function runDnsCheck() {
  ...
  const dnsChecks = await ObhodShellMethods.checkDNSAvailable();
...
// src/obhod/tabs/diagnostic/checks/runSingBoxCheck.ts
async function runSingBoxCheck() {
  ...
  const singBoxChecks = await ObhodShellMethods.checkSingBox();
  ...
  updateCheckStore({
    ...
    items: [
      ...
      },
      {
        ...
        key: _("Sing-box version is compatible (>= 1.12.0)"),
        value: ""
      },
      {
...
// src/obhod/tabs/diagnostic/checks/runNftCheck.ts
async function runNftCheck() {
  ...
  const nftablesChecks = await ObhodShellMethods.checkNftRules();
...
// src/obhod/tabs/diagnostic/checks/runFakeIPCheck.ts
async function runFakeIPCheck() {
  ...
  const routerFakeIPResponse = await ObhodShellMethods.checkFakeIP();
...
// src/obhod/tabs/diagnostic/partials/renderAvailableActions.ts
...
// src/obhod/tabs/diagnostic/partials/renderCheckSection.ts
...
// src/obhod/tabs/diagnostic/partials/renderRunAction.ts
...
// src/obhod/tabs/diagnostic/partials/renderSystemInfo.ts
...
// src/obhod/tabs/diagnostic/partials/renderWikiDisclaimer.ts
function renderWikiDisclaimer(kind) {
  ...
  return E("div", { class: className }, [
    ...
    renderButton({
      ...
      text: _("Visit Wiki"),
      onClick: () => window.open(
        "https://obhod.net/docs/troubleshooting/?utm_source=obhod",
        "_blank",
        "noopener,noreferrer"
      )
    })
...
// src/obhod/tabs/diagnostic/checks/runSectionsCheck.ts
  ...
  const items = await Promise.all(
    sections.data.map(async (section) => {
      async function getLatency() {
        if (section.withTagSelect) {
          const latencyGroup = await ObhodShellMethods.getClashApiGroupLatency(
            section.code
          );
        ...
        const latencyProxy = await ObhodShellMethods.getClashApiProxyLatency(
          section.code
        );
...
// src/obhod/tabs/diagnostic/initController.ts
async function fetchSystemInfo() {
  const systemInfo = await ObhodShellMethods.getSystemInfo();
  if (systemInfo.success) {
    ...
    store.set({
      diagnosticsSystemInfo: {
        loading: false,
        obhod_version: _("unknown"),
        obhod_latest_version: _("unknown"),
...
async function handleRestart() {
  ...
  try {
    await ObhodShellMethods.restart();
  } catch (e) {
...
async function handleEnable() {
  ...
  try {
    await ObhodShellMethods.enable();
  } catch (e) {
...
async function handleDisable() {
  ...
  try {
    await ObhodShellMethods.disable();
  } catch (e) {
...
async function handleShowGlobalCheck() {
  ...
  try {
    const globalCheck = await ObhodShellMethods.globalCheck();
...
async function handleViewLogs() {
  ...
  try {
    const viewLogs = await ObhodShellMethods.checkLogs();
    if (viewLogs.success) {
      ui.showModal(
        _("View logs"),
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig = await ObhodShellMethods.showSingBoxConfig();
...
function renderDiagnosticAvailableActionsWidget() {
  ...
  const obhodEnabled = Boolean(servicesInfoWidget.data.obhod);
...
function renderDiagnosticSystemInfoWidget() {
  ...
  function getObhodVersionRow() {
    const loading = diagnosticsSystemInfo.loading;
    const unknown = diagnosticsSystemInfo.obhod_version === _("unknown");
    const hasActualVersion = Boolean(diagnosticsSystemInfo.obhod_latest_version) && diagnosticsSystemInfo.obhod_latest_version !== "unknown";
    const version = normalizeCompiledVersion(
      diagnosticsSystemInfo.obhod_version
    );
    ...
    if (version !== `v${diagnosticsSystemInfo.obhod_latest_version}`) {
      logger.debug(
        "[DIAGNOSTIC]",
        "diagnosticsSystemInfo",
        diagnosticsSystemInfo
  ...
  const renderedSystemInfo = renderSystemInfo({
    items: [
      ...
      {
        key: "Luci App",
        value: normalizeCompiledVersion(OBHOD_LUCI_APP_VERSION)
      },
      {
        key: "Sing-box",
...
// src/obhod/tabs/diagnostic/styles.ts
var styles4 = `

#cbi-obhod-diagnostic-_mount_node > div {
    width: 100%;
}

#cbi-obhod-diagnostic > h3 {
    display: none;
}
...
// src/obhod/tabs/diagnostic/index.ts
var DiagnosticTab = {
  render: render2,
...
#cbi-obhod-settings > h3 {
    display: none;
}

/* Hide extra H3 for sections tab */
#cbi-obhod-section > h3:nth-child(1) {
    display: none;
}

/* Vertical align for remove section action button */
#cbi-obhod-section > .cbi-section-remove {
    margin-bottom: -32px;
}
